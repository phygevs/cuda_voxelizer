...
std::unique_ptr<trimesh::TriMesh> util::prepare_model(const boost::filesystem::path& path_to_model)
{
	...
	BOOST_LOG_TRIVIAL(info) << "## READ MESH" << endl;
#ifdef _DEBUG
	trimesh::TriMesh::set_verbose(true);
#endif
	BOOST_LOG_TRIVIAL(debug) << "[I/O] Reading mesh from " << path_to_model << endl;
	...
	BOOST_LOG_TRIVIAL(debug) << "[Mesh] Number of triangles: " << themesh->faces.size() << endl;
	BOOST_LOG_TRIVIAL(debug) << "[Mesh] Number of vertices: " << themesh->vertices.size() << endl;
	BOOST_LOG_TRIVIAL(debug) << "[Mesh] Computing bbox" << endl;
...
voxinfo util::voxelization_setup(const trimesh::TriMesh* mesh, const glm::uvec3& grid_sizes)
{
	...
	BOOST_LOG_TRIVIAL(info) << "## VOXELISATION SETUP" << endl;
